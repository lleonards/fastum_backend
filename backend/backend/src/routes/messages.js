...
// GET /api/messages/:slug — Pega mensagem pública por slug (sem auth)
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (photos && photos.length > 0) {
      ...
      if (paths.length > 0) {
        await supabase.storage.from('message-photos').remove(paths)
      }
    }

    await supabase.from('message_photos').delete().eq('message_id', req.params.id)
